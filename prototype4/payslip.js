...
if(data){
  ...
  const earnings=Number(data.totalSalary || 0)+ Number(data.additions || 0) - Number(data.deductions || 0)
  document.getElementById("totalEarnings").textContent=earnings.toLocaleString();
}

function downloadPDF(){
    const content=document.getElementById("payrollDownload")
    html2pdf()
    .from(content)
    .save(`payroll- ${data.name || 'Employee'}.pdf`) 
}
